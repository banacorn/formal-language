...
data State a = State a | a :. (State a) deriving (Ord, Eq)
type Alphabet = Char
...
s0 *. s1 = 
    fromList [ q0 :. State q1 | State q0 <- toList s0, State q1 <- toList s1]
...
unionFA (DFA states0 alphabets0 transition0 (State start0) accepts0) (DFA states1 alphabets1 transition1 (State start1) accepts1) =
    DFA states alphabets0 transition start accepts
    where   states = states0 *. states1
            transition (s0 :. State s1) a = next0 :. State next1
                where   (State next0) = transition0 (State s0) a
                        (State next1) = transition1 (State s1) a
            start = start0 :. State start1
            accepts = union (states0 *. accepts1) (accepts0 *. states1)

intersectionFA :: (Ord a) => FA a -> FA a -> FA a
intersectionFA (DFA states0 alphabets0 transition0 (State start0) accepts0) (DFA states1 alphabets1 transition1 (State start1) accepts1) =
    DFA states alphabets0 transition start accepts
    where   states = states0 *. states1
            transition (s0 :. State s1) a = next0 :. State next1
                where   (State next0) = transition0 (State s0) a
